...
// Autowire is a compile-time dependency injection tool.
//
// For an overview, see https://github.com/dabbertorres/autowire/blob/master/README.md
package main
...
// packages returns the slice of packages to run autowire over based on f.
// It defaults to ".".
...
// newGenerateOptions returns an initialized autowire.GenerateOptions, possibly
// with the Header option set.
func newGenerateOptions(headerFile string) (*autowire.GenerateOptions, error) {
	opts := new(autowire.GenerateOptions)
	if headerFile != "" {
		var err error
...
func (*genCmd) Usage() string {
  ...
  Given one or more packages, gen creates the autowire_gen.go file for each.
...
func (cmd *genCmd) Execute(ctx context.Context, f *flag.FlagSet, args ...interface{}) subcommands.ExitStatus {
	...
	for _, out := range outs {
		...
		if len(out.Content) == 0 {
			// No Autowire output. Maybe errors, maybe no Autowire directives.
			continue
		}
...
func (*diffCmd) Usage() string {
  ...
  Given one or more packages, diff generates the content for their autowire_gen.go
...
func (cmd *showCmd) Execute(ctx context.Context, f *flag.FlagSet, args ...interface{}) subcommands.ExitStatus {
	...
	info, errs := autowire.Load(ctx, wd, os.Environ(), cmd.tags, packages(f))
	if info != nil {
		keys := make([]autowire.ProviderSetID, 0, len(info.Sets))
		...
		if len(info.Injectors) > 0 {
			injectors := append([]*autowire.Injector(nil), info.Injectors...)
...
func (*checkCmd) Usage() string {
  ...
  Given one or more packages, check prints any type-checking or Autowire errors
...
func (cmd *checkCmd) Execute(ctx context.Context, f *flag.FlagSet, args ...interface{}) subcommands.ExitStatus {
	...
	_, errs := autowire.Load(ctx, wd, os.Environ(), cmd.tags, packages(f))
	if len(errs) > 0 {
		logErrors(errs)
		log.Println("error loading packages")
		return subcommands.ExitFailure
...
type outGroup struct {
	...
	outputs *typeutil.Map // values are *autowire.Provider, *autowire.Value, or *autowire.Field
}
...
func gather(info *autowire.Info, key autowire.ProviderSetID) (_ []outGroup, imports map[string]struct{}) {
	set := info.Sets[key]
	hash := typeutil.MakeHasher()

	// Find imports.
	next := []*autowire.ProviderSet{info.Sets[key]}
	visited := make(map[*autowire.ProviderSet]struct{})
