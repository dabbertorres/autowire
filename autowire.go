...
// Package autowire contains directives for Wire code generation.
// For an overview of working with Wire, see the user guide at
// https://github.com/dabbertorres/autowire/blob/master/docs/guide.md
//
...
// a future version of Wire: new providers sets should use autowire.Struct.
...
//		autowire.Build(otherpkg.FooSet, myProviderFunc)
//		return nil, nil
//	}
//
//	func injector(ctx context.Context) (*sql.DB, error) {
//		panic(autowire.Build(otherpkg.FooSet, myProviderFunc))
//	}
func Build(...interface{}) string {
	return "implementation not generated, run autowire"
}
...
//	func (MyFoo) Foo() {}
//
//	var MySet = autowire.NewSet(
//		autowire.Struct(new(MyFoo))
//		autowire.Bind(new(Fooer), new(MyFoo)))
...
// Example:
//
//	var MySet = autowire.NewSet(autowire.Value([]string(nil)))
...
// Example:
//
//	var MySet = autowire.NewSet(autowire.InterfaceValue(new(io.Reader), os.Stdin))
...
//    MyBar *Bar
//  }
//  var Set = autowire.NewSet(autowire.Struct(new(S), "MyFoo")) -> inject only S.MyFoo
//  var Set = autowire.NewSet(autowire.Struct(new(S), "*")) -> inject all fields
...
//
//  func NewStruct() S { /* ... */ }
//  var Set = autowire.NewSet(autowire.FieldsOf(new(S), "MyFoo", "MyBar"))
//
//  or
//
//  func NewStruct() *S { /* ... */ }
