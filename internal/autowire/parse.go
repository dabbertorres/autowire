...
func (p *providerSetSrc) description(fset *token.FileSet, typ types.Type) string {
	...
	case p.AutoBinding != nil:
		return fmt.Sprintf("autowire.AutoBind (%s)", fset.Position(p.AutoBinding.Pos))
	case p.Binding != nil:
		return fmt.Sprintf("autowire.Bind (%s)", fset.Position(p.Binding.Pos))
	case p.Value != nil:
		return fmt.Sprintf("autowire.Value (%s)", fset.Position(p.Value.Pos))
	case p.Import != nil:
	...
	case p.Field != nil:
		return fmt.Sprintf("autowire.FieldsOf (%s)", fset.Position(p.Field.Pos))
	}
...
type ProviderSet struct {
	// Pos is the position of the call to autowire.NewSet or autowire.Build that
	// created the set.
	Pos token.Pos
...
type Value struct {
	...
	// expr is the expression passed to autowire.Value.
	expr ast.Expr
...
func (oc *objectCache) processNewSet(info *types.Info, pkgPath string, call *ast.CallExpr, args *InjectorArgs, varName string) (*ProviderSet, []error) {
	// Assumes that call.Fun is autowire.NewSet or autowire.Build.
...
// autowire.Struct syntax for those.
func processStructLiteralProvider(fset *token.FileSet, typeName *types.TypeName) (*Provider, []error) {
	...
	fmt.Fprintf(os.Stderr,
		"Warning: %v, see https://godoc.org/github.com/dabbertorres/autowire#Struct for more information.\n",
		notePosition(fset.Position(pos),
			fmt.Errorf("using struct literal to inject %s is deprecated and will be removed in the next release; use autowire.Struct instead",
				typeName.Type())))
	provider := &Provider{
		Pkg:      typeName.Pkg(),
		Name:     typeName.Name(),
...
func processStructProvider(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Provider, error) {
	// Assumes that call.Fun is autowire.Struct.
...
// Since this is the only tag used by autowire, we can do string comparison
...
func processAutoBind(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*AutoBinding, error) {
	// Assumes that call.Fun is autowire.AutoBind.
...
// processValue creates a value from a autowire.Value call.
func processValue(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Value, error) {
	// Assumes that call.Fun is autowire.Value.
	...
	// Result type can't be an interface type; use autowire.InterfaceValue for that.
...
// processInterfaceValue creates a value from a autowire.InterfaceValue call.
func processInterfaceValue(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Value, error) {
	// Assumes that call.Fun is autowire.InterfaceValue.
...
// processFieldsOf creates a slice of fields from a autowire.FieldsOf call.
func processFieldsOf(fset *token.FileSet, info *types.Info, call *ast.CallExpr) ([]*Field, error) {
	// Assumes that call.Fun is autowire.FieldsOf.
	...
	for i := 1; i < len(call.Args); i++ {
		...
		if isPtrToStruct {
			...
			// autowire.Fields also provides a pointer to the field.
...
func checkField(f ast.Expr, st *types.Struct) (*types.Var, error) {
	...
	for i := 0; i < st.NumFields(); i++ {
		if strings.EqualFold(strconv.Quote(st.Field(i).Name()), b.Value) {
			if isPrevented(st.Tag(i)) {
				return nil, fmt.Errorf("%s is prevented from injecting by autowire", b.Value)
			}
...
// findInjectorBuild returns the autowire.Build call if fn is an injector template.
...
func findInjectorBuild(info *types.Info, fn *ast.FuncDecl) (*ast.CallExpr, error) {
	...
	if invalid {
		return nil, errors.New("a call to autowire.Build indicates that this function is an injector, but injectors must consist of only the autowire.Build call and an optional return")
	}
...
func isWireImport(path string) bool {
	...
	return path == "github.com/dabbertorres/autowire"
}
...
// can be a *Provider (a provider function), a *Value (autowire.Value), or an
...
// bindShouldUsePointer loads the autowire package the user is importing from their
// injector. The call is a autowire marker function call.
func bindShouldUsePointer(info *types.Info, call *ast.CallExpr) bool {
	// These type assertions should not fail, otherwise panic.
	fun := call.Fun.(*ast.SelectorExpr)                 // autowire.Bind
	pkgName := fun.X.(*ast.Ident)                       // autowire
	wireName := info.ObjectOf(pkgName).(*types.PkgName) // autowire package
