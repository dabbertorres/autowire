...
// Package autowire provides compile-time dependency injection logic as a
// Go library.
package autowire
...
func Generate(ctx context.Context, wd string, env []string, patterns []string, opts *GenerateOptions) ([]GenerateResult, []error) {
	...
	for i, pkg := range pkgs {
		...
		generated[i].OutputPath = filepath.Join(outDir, opts.PrefixOutputFile+"autowire_gen.go")
		g := newGen(pkg)
		...
		goSrc := g.frame(opts.Tags, opts.NoAddGenerateDirective)
...
func (g *gen) frame(tags string, noAddGenerateDirective bool) []byte {
	if g.buf.Len() == 0 {
		return nil
	}
