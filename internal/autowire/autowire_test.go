...
func TestWire(t *testing.T) {
	...
	wireGo, err := ioutil.ReadFile(filepath.Join("..", "..", "autowire.go"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			gopath, err := ioutil.TempDir("", "autowire_test")
			if err != nil {
				t.Fatal(err)
			}
			...
			if len(errs) > 0 {
				...
				if !test.wantWireError {
					t.Fatal("Did not expect errors. To -record an error, create want/autowire_errs.txt.")
				}
				if *record {
					wireErrsFile := filepath.Join(testRoot, test.name, "want", "autowire_errs.txt")
					if err := ioutil.WriteFile(wireErrsFile, []byte(strings.Join(gotErrStrings, "\n\n")), 0666); err != nil {
						t.Fatalf("failed to write autowire_errs.txt file: %v", err)
					}
				} else {
					if diff := cmp.Diff(gotErrStrings, test.wantWireErrorStrings); diff != "" {
						t.Errorf("Errors didn't match expected errors from autowire_errs.txt:\n%s", diff)
					}
				}
				return
			}
			if test.wantWireError {
				t.Fatal("autowire succeeded; want error")
			}
			outPathSane := true
			...
			if *record {
				// Record ==> Build the generated Autowire code,
				// check that the program's output matches the
				// expected output, save autowire output on
				// success.
				if !outPathSane {
					return
				}
				if err := gen.Commit(); err != nil {
					t.Fatalf("failed to write autowire_gen.go to test GOPATH: %v", err)
				}
				if err := goBuildCheck(goToolPath, gopath, test); err != nil {
					t.Fatalf("go build check failed: %v", err)
				}
				testdataWireGenPath := filepath.Join(testRoot, test.name, "want", "autowire_gen.go")
				if err := ioutil.WriteFile(testdataWireGenPath, gen.Content, 0666); err != nil {
					t.Fatalf("failed to record autowire_gen.go to testdata: %v", err)
				}
			} else {
				...
				if !bytes.Equal(gen.Content, test.wantWireOutput) {
					...
					t.Fatalf("autowire output differs from golden file. If this change is expected, run with -record to update the autowire_gen.go file.\n*** got:\n%s\n\n*** want:\n%s\n\n*** diff:\n%s", gotS, wantS, diff)
				}
			}
		})
	}
...
//
//			autowire_errs.txt
//					Expected errors from the Autowire Generate function,
...
//
//			autowire_gen.go
//					verified output of autowire from a test run with
//					-record, missing if autowire_errs.txt is present
//
//			program_out.txt
//					expected output from the final compiled program,
//					missing if autowire_errs.txt is present
//
func loadTestCase(root string, wireGoSrc []byte) (*testCase, error) {
	...
	wireErrb, err := ioutil.ReadFile(filepath.Join(root, "want", "autowire_errs.txt"))
	...
	if wantWireError {
		...
		if !*record {
			wantWireOutput, err = ioutil.ReadFile(filepath.Join(root, "want", "autowire_gen.go"))
			if err != nil {
				return nil, fmt.Errorf("load test case %s: %v, if this is a new testcase, run with -record to generate the autowire_gen.go file", name, err)
			}
		}
	...
	goFiles := map[string][]byte{
		"github.com/dabbertorres/autowire/autowire.go": wireGoSrc,
	}
...
func (test *testCase) materialize(gopath string) error {
	...
	// Add go.mod files to example.com and github.com/dabbertorres/autowire.
	const importPath = "example.com"
	const depPath = "github.com/dabbertorres/autowire"
