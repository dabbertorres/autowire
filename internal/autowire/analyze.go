...
func buildProviderMap(fset *token.FileSet, hasher typeutil.Hasher, set *ProviderSet) (*typeutil.Map, *typeutil.Map, []error) {
	...
	for _, b := range set.Bindings {
		...
		if concrete == nil {
			...
			ec.add(notePosition(fset.Position(b.Pos), fmt.Errorf("autowire.Bind of concrete type %q to interface %q, but %s does not include a provider for %q", b.Provided, b.Iface, setName, b.Provided)))
			continue
		}
