...
func injectFooBar() FooBar {
	autowire.Build(
		...
		autowire.Value("unused"),             // not needed -> error
		unusedSet,                        // nothing in set is needed -> error
		autowire.Bind(new(Fooer), new(*Foo)), // binding to Fooer is not needed -> error
		autowire.FieldsOf(new(S), "Cfg"),     // S.Cfg not needed -> error
		autowire.Struct(new(FooBar), "MyFoo", "MyBar"), // needed for FooBar
	)
	return FooBar{}
}
