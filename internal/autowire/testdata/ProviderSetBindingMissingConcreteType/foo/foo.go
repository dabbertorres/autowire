...
var (
	setA = autowire.NewSet(newFoo)
	// This set is invalid because it has a autowire.Bind but no matching provider.
	// From the user guide:
	// Any set that includes an interface binding must also have a provider in
	// the same set that provides the concrete type.
	setB = autowire.NewSet(autowire.Bind(new(fooer), new(*foo)))
	setC = autowire.NewSet(setA, setB)
)
