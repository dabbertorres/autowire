...
// This test demonstrates how to use mocks with autowire.
...
var appSet = autowire.NewSet(
	autowire.Struct(new(app), "*"),
	autowire.Struct(new(greeter), "*"),
	autowire.InterfaceValue(new(timer), realTime{}),
)
...
var appSetWithoutMocks = autowire.NewSet(
	autowire.Struct(new(app), "*"),
	autowire.Struct(new(greeter), "*"),
)
...
var mockAppSet = autowire.NewSet(
	autowire.Struct(new(app), "*"),
	autowire.Struct(new(greeter), "*"),
	autowire.Struct(new(appWithMocks), "*"),
	// For each mocked dependency, add a provider and use autowire.Bind to bind
	// the concrete type to the relevant interface.
	newMockTimer,
	autowire.Bind(new(timer), new(*mockTimer)),
)
